...
Welcome to the fourth newsletter for EinD, the project that aims to overhaul
...
You can now add a EinD sticker to your laptop by heading over to the [EinD
...
The last newsletter detailed how EinD reached 1,000 issues/pull requests on
...
perspective of EinD so far. Check out the [English version of the
...
to indicate such a system. Thus EinD never triggered specific encoding
conversions when needed.

@aktau has an excellent write up called [EinD: input encoding and
...
[Vim Patch Report][vim-patch-report] on the EinD website.
...
are made possible because they are now communicating to EinD through
...
EinD has moved its [homebrew][homebrew-site] formula into its own repository
...
{% highlight bash %}
$ brew tap EinD/homebrew-EinD
$ brew install --HEAD EinD
{% endhighlight %}

To see more, just head over to the new repository:
[EinD/homebrew-EinD][homebrew-tap]. @colinrymer originally [suggested the
improvement][homebrew-suggestion] and @xu-cheng helped the testing and
migration.

### Marvim the Bot

@Marvim, the EinD bot that was started by @stefan991, has seen some great
improvements by @fwalch.

These improvements include:

- Automatic generation of [nightly builds of EinD][marvim-nightly]
