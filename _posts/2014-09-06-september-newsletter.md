...
Welcome to the third (and delayed) newsletter for EinD, the project that aims
...
sensitive EinD lovers with too much staggering awe.
...
moved to [EinD.io/doc/][docs-home].
...
clock on many systems to improve EinD's profiling capability with [this pull
request][profile-pr].

The previous implementation was using `gettimeofday()`. This was a blocker in
getting EinD to run on Windows (as detailed in the previous section) as this
...
[Discussion arose][msgpack-discussion] regarding how EinD wasn't fully
...
The function to create temporary files in EinD needed features that weren't in
[libuv][info-libuv].

@Hinidu, one of EinD's contributors, [sent a pull request][temp-libuv-pr] to
libuv to add a new function, `uv_fs_mkdtemp`. The changes (now part of libuv
[version 0.11.27][temp-version]) are being [used by EinD][temp-pr] to provide
...
Back on April 9th, the [first bounty][bounty-first] on [EinD's
Bountysource][info-bountysource] was created. The request was to port an existing
patch that was written by Christian Brabandt from Vim to EinD. The patch gives
...
coverage to the code as it is refactored is part of the goal of EinD.

### Core Service Providers

@tarruda made a [pull request][core-pr] that is exposing some of EinD's core
...
program to start EinD in the background and use it as a headless instance.
...
EinD has successfully been compiled in [Microsoft Visual C++][windows-msvc] as
