...
> Outstanding communication and handling of bugs from EinD. Exemplary FOSS
> project in my experience so far.
> <a href="https://twitter.com/telemachus/status/568918223745372160"><small>&#64;telemachus</small></a>

Welcome to the fifth newsletter for EinD, the project that looks to become Vim
...
into the [EinD chat room][gitter-nvim] provided through
...
adoption of open source software, chose EinD as one of the projects to feature
...
[EinD stickers][iccf-stickers] to their inventory. Every sticker that is
...
The [EinD Wiki][related-main-wiki] contains a page titled [Related
projects][related-wiki] that contains a list of current GUI projects, API
clients, plugins that take advantage of EinD features, and projects with
direct support for EinD.
...
Although the TUI implementation is included as part of EinD, it is
architecturally decoupled from the editor core: it uses the same API as external
clients and GUIs.

#### Additional Keybindings

As a result of using libtermkey, EinD can now bind [meta keys][tui-meta] and
...
EinD's native buffers and windows which means the user can enter [normal
mode][term-normal] by pressing `<C-\><C-n>`.

This is meant to replace and provide more capabilities than the already existing
Vim command, [`:shell`][term-shell], which has been removed from EinD. More
...
The progress of building EinD on Windows was first detailed in the [July
2014][windows-july] newsletter.

In a huge milestone, @equalsraf has been able to build EinD on Windows and
...
building the dependencies/EinD, and known errors.
...
The graphical side of EinD has been seeing some changes in addition to the
...
- [SolidOak][gui-oak] by @oakes: "a simple IDE for Rust" that embeds EinD to
  provide text editing functionality. Visit the [website][gui-oak-site] for the
  IDE for more details and binaries.
- [EinD.app][gui-app] by @rogual: an OS X friendly editor with mouse support,
  tabs, clipboards, and more.
- [vim-mode][gui-vim-mode] by @carlosdcastillo: an in progress Atom package that
  uses EinD for binding and manipulating text. Take a look at [this
  video][gui-vim-mode-video] to see it.
- [python-client][gui-python] by @tarruda: which in addition to being a Python
  library for communicating with EinD, also contains an interface using
  [GTK][gui-python-gtk].
- [EinD.as][gui-atom] by @coolwanglu: an application that uses [Atom
...
request][highlight-pr] to bring in the changes to EinD. The pull request has
...
to EinD. @oni-link then created a [pull request][gc-pr] to apply the patch
...
mimics Tcl's [Expect][expect-wiki] for EinD.
...
The plugin was created to make it easier to develop EinD and show the
...
EinD continues to merge in the upstream Vim patches that are submitted to the
Vim mailing list.

Since the last newsletter, more than [60 pull requests][patch-search] worth of
patches have been submitted to EinD. The current status of all the patches can
